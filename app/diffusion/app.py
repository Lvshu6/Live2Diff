...
def load_wan_pipe():
    ...
    print("WanFlow model loaded successfully.\n")
    return pipe
...
# ==================== Trajectory Management & Drawing Logic ====================
...
def undo_last_point_from_current_trajectory(original_image, trajectories, current_traj_idx, temp_point):
    ...
    if len(current_trajectory) > 0:
        ...
        print(f"✅ Undid last point in trajectory group {current_traj_idx}.")
    else:
        print(f"⚠️ Trajectory group {current_traj_idx} is empty, cannot undo.")
    updated_img = draw_trajectories_on_image(original_image, trajectories, current_traj_idx, temp_point)
    return updated_img, trajectories, current_traj_idx, temp_point
...
def generate_output_new(trajectories_list, original_image_state, use_wan,num_frames):
    ...
    if use_wan:
        print("🎬 Starting WanFlow video generation...")
        path = wan_flow(original_image_state, path)
    else:
        print("⏭️ Skipping WanFlow, output trajectory visualization only.")
        
    return path

# ==================== Gradio UI ====================
with gr.Blocks() as demo:
    gr.Markdown("## 🖱️Live2Diff: Trajectory Annotation(Click Version)")
    gr.Markdown("Frame format: 1+4K → only 5, 9, 13, 17 allowed")
    ...
    with gr.Row():
        with gr.Column():
            img_display = gr.Image(label="Upload image and annotate here", interactive=True, type="pil")
            
            # ✅ Frame count selector (1+4K format)
            frame_selector = gr.Radio(
                choices=[5, 9, 13, 17],
                value=9,
                label="Select Frame Count (1+4K)",
                interactive=True
            )
            
            wan_checkbox = gr.Checkbox(label="Enable Live2Diff Video Generation (slow)", value=False)
            
            with gr.Row():
                clear_btn = gr.Button("Clear All")
                new_traj_btn = gr.Button("New Trajectory")
                undo_traj_btn = gr.Button("Undo Last Point")
